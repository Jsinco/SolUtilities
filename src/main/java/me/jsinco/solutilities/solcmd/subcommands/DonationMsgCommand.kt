...
class DonationMsgCommand : SubCommand {
    ...
    override fun execute(plugin: SolUtilities, sender: CommandSender, args: Array<out String>) {
        if (args.size < 3) {
            sender.sendMessage(ColorUtils.colorcode("${plugin.config.getString("prefix")}Invalid usage. §c/solutilities donationmsg <player?> <package!!>"))
            return
        }

        val player = Bukkit.getOfflinePlayer(args[1])
        val packageType: String = ColorUtils.colorcode(args.joinToString(" ").replace(args[0], "").replace(args[1], "").trim())
        Bukkit.broadcastMessage(IridiumColorAPI.process(ColorUtils.colorcode("&#f76a3b&lS&#f7863b&lt&#f8a33b&lo&#f8bf3a&lr&#f8db3a&le &8» <GRADIENT:FFAA00>&l${player.name}</GRADIENT:FDD394> &#E2E2E2has just purchased a ${packageType}&#E2E2E2! Thank you for supporting &#f76a3b&lS&#f77a3b&lo&#f78a3b&ll&#f79a3b&la&#f8ab3a&lc&#f8bb3a&le&#f8cb3a&lM&#f8db3a&lC&r&f!")))
